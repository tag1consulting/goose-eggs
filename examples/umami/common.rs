...
pub async fn search(user: &GooseUser, english: bool) -> GooseTaskResult {
    ...
    let search_page = goose_eggs::validate_and_load_static_assets(
        user,
        goose,
        &goose_eggs::Validate::title(if english { "Search" } else { "Buscar" }),
    )
    .await?;

    let form = goose_eggs::drupal::get_form(&search_page, "search-form");
    let form_build_id = goose_eggs::drupal::get_form_value(&form, "form_build_id");
    let form_id = goose_eggs::drupal::get_form_value(&form, "form_id");
    ...
    goose = user.goose_send(request_builder.form(&params), None).await?;

    let _search_results = goose_eggs::validate_and_load_static_assets(
        user,
        goose,
        &goose_eggs::Validate::text(&search_phrase),
    )
    .await?;
